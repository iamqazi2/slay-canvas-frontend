...
//   onDrop: (e: React.DragEvent, videoId: string) => void;
//   onDragEnd: () => void;
//   isDragging: boolean;
//   onExternalDragStart?: (e: React.DragEvent, video: VideoItem) => void;
// }> = ({ video, onRemove, onDragStart, onDragOver, onDrop, onDragEnd, isDragging, onExternalDragStart }) => {
//   const getPlatformColor = (type: VideoItem["type"]) => {
//     switch (type) {
//       case "youtube":
//         return "#ff0000";
//       case "vimeo":
...
//   const getPlatformIcon = (type: VideoItem["type"]) => {
//     switch (type) {
//       case "youtube":
//         return (
...
//     e.dataTransfer.setData("application/video-item", JSON.stringify(video));
//     e.dataTransfer.setData("text/plain", video.url);
//     e.dataTransfer.effectAllowed = "copy";

//     // Call both internal and external drag handlers
//     onDragStart(e, video.id);
//     if (onExternalDragStart) {
//       onExternalDragStart(e, video);
//     }
//   };

//   return (
//     <div
//       className={`relative bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-200 ${
...
//         style={{
//           background: getPlatformColor(video.type),
//         }}
//       >
...
//           <span className="text-white font-medium text-sm">{video.platform}</span>
//         </div>
...
//         {/* Title */}
//         <h3 className="text-lg font-bold text-gray-900 mb-2 truncate" style={{ fontFamily: "Arial, sans-serif" }}>
//           {video.title}
//         </h3>

//         {/* Description */}
//         <p className="text-sm text-gray-600 mb-4 leading-relaxed">{video.description}</p>

//         {/* Video Preview Area */}
//         <div className="relative bg-gray-200 rounded-lg overflow-hidden mb-4 flex-1" style={{ minHeight: "80px" }}>
//           {video.type === "youtube" ? (
//             <iframe
//               src={
...
//           ) : video.type === "vimeo" ? (
//             <iframe
//               src={`https://player.vimeo.com/video/${video.url.split("vimeo.com/")[1]?.split("?")[0]}`}
//               className="w-full h-full"
//               frameBorder="0"
...
//                 <p className="text-xs opacity-75 mt-1">Click to open</p>
//               </div>
//             </div>
//           )}
//         </div>
...
export default function VideoCollection({
  ...
  //   dispatch(setVideoCollection(videos.filter((video) => video.id !== videoId)));
  // };
  ...
  //   dispatch(setVideoCollection(newVideos));
  // };
  ...
  //     const fromIndex = videos.findIndex((v) => v.id === draggedVideoId);
  //     const toIndex = videos.findIndex((v) => v.id === targetVideoId);
  ...
  // const handleExternalDragStart = useCallback((e: React.DragEvent, video: VideoItem) => {
  ...
  //     width: 200px;
  //     height: 120px;
  //     background: linear-gradient(135deg, ${getPlatformColor(video.type)} 0%, ${getPlatformColor(video.type)}dd 100%);
  //     border-radius: 12px;
  //     padding: 12px;
  //     color: white;
  ...
  //   dragImage.innerHTML = `
  //     <div style="font-size: 14px; font-weight: bold; margin-bottom: 4px;">${video.title}</div>
  //     <div style="font-size: 12px; opacity: 0.9;">${video.platform}</div>
  //     <div style="font-size: 10px; opacity: 0.7; margin-top: 4px;">Drop anywhere to create new component</div>
  //   `;
          ...
          >
                    ...
                    <div className="p-2 text-white h-full">
                      <div className="h-full flex flex-col">
                        ...
                        >
                            ...
                            >
                              ...
                              <div className="flex items-center space-x-2">
                                ...
                                <div>
                                  ...
                                  <p className="text-white/80 text-xs">instagram.com</p>
                                </div>
                              </div>
                              <div className="flex items-center space-x-2">
                          ...
                          {/* Video Title Overlay */}
                          {/* <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/70 to-transparent p-2 z-10">
                          <h3 className="text-white text-sm font-medium truncate mb-1">{getVideoTitle(videoUrl)}</h3>
                          <p className="text-white/70 text-xs truncate">{videoUrl}</p>
                        </div> */}
                        </div>
            ...
            <div className="relative">
              <svg
                ...
                xmlns="http://www.w3.org/2000/svg"
              >
                <g filter="url(#filter0_d_62_4648)">
                  <circle cx="17.793" cy="17.7633" r="11.5294" fill="#F0F5F7" />
                  <circle
                    cx="17.793"
                    cy="17.7633"
                    r="11.1691"
                    stroke="#4596FF"
                    strokeOpacity="0.1"
                    strokeWidth="0.720586"
                  />
                </g>
                <defs>
                  <filter
                    id="filter0_d_62_4648"
                    x="0.498986"
                    y="0.469201"
                    width="34.588"
                    height="34.5881"
                  ...
                  >
                    ...
                    />
                    <feOffset />
                    <feGaussianBlur stdDeviation="2.88234" />
                    ...
                    <feBlend
                      ...
                      result="effect1_dropShadow_62_4648"
                    />
                    <feBlend
                      mode="normal"
                      in="SourceGraphic"
                      in2="effect1_dropShadow_62_4648"
                      result="shape"
                    />
                  </filter>
                </defs>
              ...
              >
                <path
                  d="M1.50076 1.49989C66.3873 23.881 111.432 42.707 115.001 85.4999"
                  stroke="#1279FF"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeDasharray="7 7"
...
import { useVideoDrop, VideoItem } from './VideoCollection';

function MyComponent() {
  const handleVideoDrop = (video: VideoItem, position: { x: number; y: number }) => {
    console.log('Video dropped:', video);
    console.log('Drop position:', position);
    
    // Create a new component or handle the video
    // For example, create a new AudioPlayer, ImageCollection, etc.
    const createEvent = new CustomEvent("createComponent", {
      detail: {
        componentType: "audioPlayer", // or any other component type
        data: { video }, // Pass the video data
        position: position, // Optional: position where it was dropped
      },
    });
    window.dispatchEvent(createEvent);
  };

  useVideoDrop(handleVideoDrop);

  return (
    <div className="drop-zone">
      <p>Drop videos here to create new components!</p>
    </div>
  );
}
*/
