...
const WikipediaLink: React.FC<WikipediaLinkProps> = ({
  ...
  // Handle routed text content from VideoCollection (for backward compatibility)
  useEffect(() => {
    const handleTextPasted = (event: CustomEvent) => {
      const text = event.detail.text;
      if (text && text.trim()) {
        // Update the Wikipedia link to show the pasted text
        // You can modify this to handle the text as needed
        console.log("Text content received:", text);

        // For now, just log the text - you can extend this to process the text
        // This could involve searching for Wikipedia articles, displaying the text, etc.
      }
    };
  ...
  const generateWikipediaLink = (text: string): string => {
    // Clean the text and create a Wikipedia search URL
    if (text?.startsWith("http")) return "";
    const cleanText = text
      ?.trim()
      ?.replace(/[^\w\s]/g, "")
      ?.replace(/\s+/g, "_");
    return `https://en.wikipedia.org/wiki/${encodeURIComponent(cleanText)}`;
  };
