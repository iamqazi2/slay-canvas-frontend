...
const Sec1Btn: React.FC<ButtonProps> = ({ text, text2, width, link, link2, variant1, variant2, icon1, icon2 }) => {
  return (
    <motion.div
    ...
    >
      ...
      {link ? (
        <Link href={link}>
          <button
            className={`flex items-center gap-2 px-3 text-xs md:px-4.5 py-[11px] rounded-xl font-medium sm:text-sm cursor-pointer
      ${variant1 === "default"
    ...
    `}
          >
            {icon1 && <Image src={icon1} alt="btn-icon1" height={15} width={15} />}
            {text}
          </button>
        </Link>
      ) : (
    ...
    `}
      ...
      {link2 ? (
        ...
        <button
          ...
          style={{ width }}
        >
          {icon2 && <Image src={icon2} alt="btn-icon2" height={15} width={15} />}
          {text2}
        </button>
      )}
    </motion.div>
